...
setMethod("length_plot",
          ...
          function(object,
            ...
            if(type == "length"){
              ...
              ptlayer <- geom_col(aes(x = qwidth, y = counts/10^6),width = col_width,fill = col_color)
              ...
              ptlayer <- geom_col(aes(x = qwidth, y = counts/10^6, fill = factor(frame)),
            ...
            p <-
              ...
              xlab("Read length (nt)") + ylab("Number of reads (10^6)")
...
setMethod("feature_plot",
          ...
          function(object,merge_rep = FALSE,
            ...
            p <-
              ggplot(sry) +
              geom_col(aes(x = type,y = counts/10^6,fill = type),width = 0.6,show.legend = F) +
              facet +
              theme(panel.grid = element_blank(),
                    strip.text = element_text(colour = "black",face = "bold",size = rel(1)),
                    axis.text = element_text(colour = "black")) +
...
setMethod("relative_dist_plot",
          ...
          function(object,
            ...
            p <-
              ggplot(pltdf) +
              geom_col(aes(x = rel, y = counts/10^6, fill = factor(frame)),
              ...
              scale_fill_manual(values = c("0" = "#003366", "1" = "#336699", "2" = "#CCCCCC"),
                                name = "frame") +
              xlab("Distance to start/stop codon (nt)") + ylab("Number of reads (10^6)")
...
setMethod("relative_offset_plot",
          ...
          function(object,
            ...
            p <-
              ggplot(summary.info) +
              geom_path(aes(x = rel_pos,y = counts/10^6)) +
              mx.label +
              ...
              # scale_y_continuous(labels = scales::label_log(base = 10,digits = 1)) +
              xlab(xlab) +
              ylab("Number of reads length (10^6)")
