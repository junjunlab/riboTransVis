...
#' @title Calculate Normalized Reads (TPM or RPKM)
#'
#' @description
#' This function calculates normalized expression values (TPM or RPKM) for RNA-seq
#' and Ribo-seq data stored within a `ribotrans` object. It can also compute
#' Translation Efficiency (TE) based on these normalized values.
#'
#' @details
#' The function first checks if raw counts have been generated by `get_counts`.
#' It then proceeds to calculate normalization for the specified data types (`rna`,
#' `ribo`).
#'
#' For TPM (Transcripts Per Million) calculation, the formula is:
#' 1. RPK (Reads Per Kilobase) = `raw_counts / (gene_length / 1000)`
#' 2. Per-million scaling factor = `sum(RPK)` for all genes in the sample
#' 3. TPM = `(RPK / scaling_factor) * 1e6`
#'
#' For RPKM (Reads Per Kilobase of transcript per Million mapped reads), the
#' formula is:
#' RPKM = `(raw_counts / (gene_length / 1000)) / (total_mapped_reads / 1e6)`
#'
#' Translation Efficiency (TE) is calculated as the ratio of normalized Ribo-seq
#' reads to normalized RNA-seq reads (`TPM_ribo / TPM_rna` or
#' `RPKM_ribo / RPKM_rna`) for genes common to both datasets.
#'
#' The method handles data processed via both genome and transcriptome mapping
#' (`object@mapping_type`), adjusting how gene lengths are retrieved accordingly.
#'
#' @param object A `ribotrans` object that has been processed by `get_counts()`
#'   and contains raw count matrices.
#' @param norm_type A character string specifying the normalization method.
#'   Must be one of `"tpm"` (default) or `"rpkm"`.
...
#' @param type A character vector specifying which data types to process. Can
#'   contain any combination of `"rna"`, `"ribo"`, and `"te"`. For example,
#'   `type = c("rna", "ribo", "te")` will calculate and return all three.
#' @param ... Additional arguments (not currently used).
#'
#' @return
#' A list containing the requested data frames. The names of the list elements
#' depend on the `norm_type` and `type` parameters.
#' \itemize{
...
#' (e.g., `gene_id`, `gene_name`, `gene_biotype`).
#'
#' @export
          ...
          function(object,
            ...
            if("rna" %in% type){
              if(object@mapping_type == "genome"){
                ...
                rnainfo <- subset(lib, type == "rna")
                rnainfo <- rnainfo[match(colnames(count_rna),rnainfo$sample),]
                ...
                if(norm_type == "tpm"){
                  ...
                  if(norm_by_totalMappedReads == TRUE){
                    tpm_rna <- t(t(rpk)/rnainfo$mappped_reads*10^6)
                  }else{
                  ...
                  if(norm_by_totalMappedReads == TRUE){
                    tpm_rna <- t(t(count_rna)/rnainfo$mappped_reads*10^6)
                    tpm_rna <- tpm_rna/kb
                  }else{
            ...
            if("ribo" %in% type){
              if(object@mapping_type == "genome"){
                ...
                riboinfo <- subset(lib, type == "ribo")
                riboinfo <- riboinfo[match(colnames(count_ribo),riboinfo$sample),]
                ...
                if(norm_type == "tpm"){
                  ...
                  if(ribo_count_feature == "cds"){
                    tpm_ribo <- t(t(count_ribo)/riboinfo$mappped_reads*10^6)
                    tpm_ribo <- tpm_ribo/kb
                  }else{
                    tpm_ribo <- t(t(count_ribo)/colSums(count_ribo)*10^6)
                    tpm_ribo <- tpm_ribo/kb
                  }
                ...
                # rpk <- count_ribo/kb
                # tpm_ribo <- t(t(rpk)/colSums(rpk)*10^6)
                # tpm_ribo <- t(t(rpk)/colSums(count_rna)*10^6)
